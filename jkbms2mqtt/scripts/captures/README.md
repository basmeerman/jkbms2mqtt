...
| File | What it is |
|---|---|
| `BMS_<1-6>_baseline.json` | Settings of every pack in the bank, captured read-only. Raw words plus decoded values, and the restore source if a bench run ever aborts. Packs 1-5 are identical; pack 6 is configured for 360 Ah against 314 Ah elsewhere, which is deliberate — that pack is a different capacity. Pack 1 originally had a 3.50 V cell over-voltage recovery against 3.58 V on the rest; it was aligned to 3.58 V at the owner's request and its baseline re-captured, so these files reflect the bank's current configuration. |
